...
class BaseSolver(object):
    ...
    def _setup(self, ):
        ...
        if self.teacher:
            self.teacher = dist_utils.warp_model(self.teacher.to(device), sync_bn=cfg.sync_bn, \
            find_unused_parameters=cfg.find_unused_parameters)
    ...
    def load_teacher_resume_state(self, path: str):
        """load teacher resume
        """
        if path.startswith('http'):
            state = torch.hub.load_state_dict_from_url(path, map_location='cpu')
        ...
        module = dist_utils.de_parallel(self.teacher)

        if 'model' in state:
            module.load_state_dict(state['model'], strict=True)
            print(f'Load teacher.state_dict')
            return
        else:
            if 'ema' in state:
                stat, infos = self._matched_state(module.state_dict(), state['ema']['module'])
            ...
            print(f'Number of missed keys: {len(infos["missed"])}, Number of unmatched keys: {len(infos["unmatched"])}')
            def count_params(nested_dict):
                total = 0
                if isinstance(nested_dict, dict):
                    for value in nested_dict.values():
                        total += count_params(value)
                elif isinstance(nested_dict, torch.Tensor):
                    total = nested_dict.numel()
                else:
                    total = 0
                return total
            print(f'Number of matched parameters: {count_params(stat)}')
            module.load_state_dict(stat, strict=True)
