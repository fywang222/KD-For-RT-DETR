...
#from ..transforms.v2 import ConvertImageDtype, Normalize
...
class BaseDetector(nn.Module):
    def __init__(self, min_size=None, max_size=None, size_divisible=32):
        ...
        if len(size) != 0:
            ...
            eval_transform = []
        eval_transform.append(ConvertImageDtype(torch.float))
        eval_transform.append(Normalize((0.485, 0.456, 0.406), (0.229, 0.224, 0.225), False))
        self.eval_transform = nn.Sequential(*eval_transform)
        '''
        self.eval_transform = None # FIXME
        self._device = None
